...
public sealed class DynamoWorldRepository : IWorldRepository, IAffiliationRepository
{
    private readonly IDynamoDbClient _dynamo;
    private readonly DynamoOptions _options;

    public DynamoWorldRepository(IDynamoDbClient dynamo, DynamoOptions options)
    {
        _dynamo = dynamo;
        _options = options;
    }

    public async Task<World?> GetWorldAsync(string worldId, CancellationToken cancellationToken)
    {
        EnsureTable();
    ...
    public async Task<World> CreateWorldAsync(string hostId, World world, CancellationToken cancellationToken)
    {
        EnsureTable();
    ...
    public async Task<Affiliation> CreateAffiliationAsync(Affiliation affiliation, CancellationToken cancellationToken)
    {
        ...
        var item = new Dictionary<string, AttributeValue>
        {
            ...
            ["Status"] = new AttributeValue { S = affiliation.Status.ToString() },
    ...
    private static World MapWorld(Dictionary<string, AttributeValue> item, string worldId)
    {
        return new World
        {
            Id = worldId,
    ...
    private static AffiliationStatus ParseStatus(string? value)
    {
        return Enum.TryParse<AffiliationStatus>(value, ignoreCase: false, out var parsed)
            ? parsed
